...
import { useUpdateUnit } from '../features/units/useUpdateUnit';
...
function EditUnitForm({ onCloseModal, unitData }) {
  ...
  const { updateunit, isPending, error } = useUpdateUnit();

  function onSubmit(data) {
    const finalData = { ...data, unitId: unitData._id };
      ...
      >
        <div className="mb-4 flex flex-col md:flex-row md:items-center md:gap-4">
          ...
          <select
            id="apartmentUnit"
            name="apartmentUnit"
            defaultValue={unitData?.apartmentUnit}
        ...
        <div className="mb-4 flex flex-col md:flex-row md:items-center md:gap-4">
          ...
          <select
            id="unitType"
            name="unitType"
            defaultValue={unitData?.unitType}
            className="mt-1 block w-full rounded-md border border-gray-300 px-4 py-2 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm md:w-2/3"
            {...register('unitType')}
          >
          ...
          <div className="mt-1 md:w-2/3">
            <label className="inline-flex items-center">
              <input
                ...
                id="vacant"
                name="vacant"
                defaultChecked={!unitData?.vacant}
